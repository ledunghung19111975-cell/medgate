...
function valuesFromYaml(key) {
  return [...agentsYaml.matchAll(new RegExp(`^\\s*(?:-\\s*)?${key}:\\s*(.+)$`, "gm"))].map((match) => match[1].trim());
}
...
for (const fixture of fixtures) {
  ...
  assert(["pretriage-baseline-v1", "pretriage-candidate-v2"].includes(fixture.agent_key), `unknown agent: ${fixture.agent_key}`);
  ...
  assert(fixture.judge_result.checkpoint && fixture.judge_result.evidence, `judge evidence incomplete: ${fixture.fixture_id}`);
  ...
  const oppositeRole = fixture.agent_key === "pretriage-baseline-v1" ? "候选" : "基线";
  const roleScopedEvidence = `${fixture.judge_result.auto || ""}\n${fixture.judge_result.evidence || ""}`;
  assert(!roleScopedEvidence.includes(oppositeRole), `fixture evidence references the opposite role: ${fixture.fixture_id}`, { oppositeRole });
...
console.log(JSON.stringify({
  ...
  fixtures: { total: fixtures.length, perCase: 2, agents: ["pretriage-baseline-v1", "pretriage-candidate-v2"] },
  blockingFixtures,
