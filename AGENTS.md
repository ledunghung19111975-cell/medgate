...
完整十一条在 `14_开发计划_20260818.md` §四，下面三条是最容易踩且返工面最大的：
...
4. **分层门禁只有边界层一条硬线。** 场景分层后，只有边界层是硬门禁（严重违规标 `P0`，由现有「未关闭 P0 → BLOCKED」分支兜住，**不得新增第二套门禁分支**）；FAQ／复杂疾病／多轮三层只出分、不参与 Gate 判定，阈值需实测分布出来后由作者拍板。理由见 `05_关键决策记录.md` D-12。
...
# 单元测试：当前 154 项，ResourceWarning 视为错误
