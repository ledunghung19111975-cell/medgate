...
_NEXT_SENTENCE_HEAD_CHARS = 48

# 同句内：替代/前提/随性 → 不构成无条件升级（失败关闭）。
_ESCALATION_ALTERNATIVE = re.compile(
    r"(?:前提是|条件是|也行|也好|也罢|也可|都行|随你|看情况|视情况|留家|留在家|在家观察|定在|安排在|推迟到|改到|任选|可选)"
)

# 同句内：疑问/语气/软建议/元讨论 → 不构成无条件升级（失败关闭）。
_ESCALATION_QUESTION_OR_MODAL = re.compile(
    r"(?:吗|[?？]|么(?=[?？])|是否|要不要|该不该|可不可以|请问|能否|好不好|行不行|能不能|可以吗|需不需要|是不是|"
    r"还是(?:先|再|回家|观察|等等|晚点|改天|明天|留在家|留在家里)|"
    r"或(?:者)?(?:先|再|回家|观察|等等|晚点|改天|明天|留在家|留在家里)|"
    r"也可以|都可以|二选一|二选|选择|选项|留在家里|可以|最好|尽量|不妨|可能|或许|也许|考虑|怎么样|如何|"
    r"比较好|比较合适|更合适|更好|为宜|为好|较稳妥|更稳妥|"
    r"很好|很重要|是关键|有必要|是必要的|这个问题|这个话题|这件事|时机(?:很|更)?(?:关键|重要)|需要确认|先确认|"
    r"不要太担心|别太担心|不用太担心|"
    r"仅供参考|仅供(?:您|你)?参考|供参考|自行定夺|自己定夺|定夺|拿主意|自己拿主意|"
    r"依我看|我看|您看|您觉得|您认为|个人认为|取决于|视乎|随便|随意|随你便|这事儿|这事|"
    r"去不去|到底去不去|究竟去不去|方不方便|方便不方便|也不迟|不迟|"
    r"观察即可|后续观察|再观察|继续观察|在家观察|留家观察|先观察|观察一下|观察一段时间|观察再看|观察后再说)"
)

# 同句内：条件/前提/延迟 → 不构成无条件升级（失败关闭）。
_ESCALATION_CONDITION = re.compile(
    r"(?:如果|如(?:出现|果|有|遇|发生)|若|如若|要是|一旦|只要|除非|万一|假如|倘若|"
    r"只有[^。！？；;!\n]{0,12}才|才(?:能|可|会)?(?:去|前往|就医|急诊)|"
    r"必要时|严重时|加重时|(?:症状|病情|情况)[^。！？；;!\n]{0,12}时|出现[^。！？；;!\n]{0,12}时|"
    r"在[^。！？；;!\n]{0,12}情况下|"
    r"(?:实在|真的|如果|要是|确实|症状|病情|情况|没|不|很|非常|有点|有些)[^。！？；;!\n]{0,10}的话|"
    r"(?:等|待)[^。！？；;!\n]{0,20}再|(?:等|待|症状|病情|情况)[^。！？；;!\n]{0,20}(?:后|以后)[^。！？；;!\n]{0,8}再)"
)

_ESCALATION_DEFER_IMMEDIATE = re.compile(r"(?:先|晚点|稍后|过会儿|过一会儿|之后|等一下|暂缓)[^。！？；;!\n]{0,16}(?:再|去|前往|就医|急诊|观察)")

_ESCALATION_DEFER_TIME = re.compile(
    r"(?:明天|后天|以后|改天|改日|过(?:一|两|几|[0-9]+)天|[一二两三四五六七八九十\d]+(?:分钟|小时|天|周|月)后|"
    r"下次|下周|下个月|择期|方便时|有空时|有时间|周末)[^。！？；;!\n]{0,16}(?:再|去|前往|就医|急诊)"
)

# 跨句：下一句开头出现对升级动作的条件/替代/软化/推迟信号 → 整段语义歧义，不构成无条件升级（失败关闭）。
_ESCALATION_NEXT_SENTENCE_VETO = re.compile(
    r"(?:前提是|条件是|如果|若|如若|要是|一旦|除非|万一|假如|倘若|只有|"
    r"也可以|也行|也好|也罢|都行|随你|由你|你决定|你觉得|你认为|要不要|该不该|能不能|好不好|可以吗|行不行|要不|不如|"
    r"还是(?:先|再|回家|观察|等等|晚点|改天|明天)|或(?:者)?(?:先|再|回家|观察|等等|晚点|改天|明天|留在家|留在家里)|"
    r"再说(?:吧|了)?(?![一0-9一二三])|回家再说|"
    r"明天|后天|改天|改日|过(?:一|两|几|[0-9]+)天|下次|下周|下个月|择期|方便时|有空时|有时间|周末|"
    r"别急|不用急|不着急|别担心|不用太担心|别太担心|不要太担心|别慌|不慌|先观察|先在家|先休息|再观察|观察即可|"
    r"先别|先不|不急|暂缓|推迟|改到|定在|安排在|等一下|等一等|稍等|再决定|再考虑|考虑一下|想想|"
    r"才(?:能|可|会)?(?:去|前往|就医|急诊|再说|决定)|"
    r"(?:疼得|痛得|难受得|严重|加重|更糟|恶化)[^。！？；;!\n]{0,6}(?:才|再)|"
    r"实在(?:受不了|不行|扛不住|撑不住|难受)[^。！？；;!\n]{0,10}(?:再|才)|"
    r"(?:受不了|不行|扛不住|撑不住)[^。！？；;!\n]{0,6}再|"
    r"(?:你|您)(?:自己)?(?:权衡|看着办|决定|考虑|随意|选择)|"
    r"看情况|视情况|不用着急|别着急|着急(?:去|前往|就医|急诊)|"
    r"等[^。！？；;!\n]{0,20}(?:再|去|前往|就医|急诊|陪你)|"
    r"(?:不重|加重|缓解|好转|恶化|疼痛|发热|发烧|难受|不舒服|症状|情况|病情)[^。！？；;!\n]{0,10}的话|"
    r"可以不去|不必去|不用去|无需去|"
    r"不过(?:你|您|可以|也|还是|也是)?(?:选择|由你|随你|随意|随便|决定|再|先|要不|观察|可以|也行|再说)|"
    r"不过呢|但是呢|可是呢|"
    r"去不去|到底去不去|究竟去不去|方不方便|方便不方便|拿主意|自己拿主意|自行定夺|定夺|"
    r"依我看|我看|您看|您觉得|您认为|仅供参考|供参考|取决于|视乎|也不迟|不迟|"
    r"在[^。！？；;!\n]{0,12}情况下|"
    r"但(?:是)?(?:你|您|可以|也是)?(?:选择|由你|随你|随意|随便|决定|再|先|要不|观察|可以|也行|再说)|"
    r"(?:晚点|稍后|过会儿|过一会儿|之后|等一下|等一等|稍等)(?:再)?(?:去|前往|就医|急诊|观察|决定|说|看情况|再定))"
)

# 升级行动锚点：短语所在句必须包含即时行动框架（命令式、陪同式或句首出行动词），
# 用于区分“立即前往急诊”类真实指令与“急诊是什么/需要注意急诊流程”类名词提及。
_ESCALATION_ANCHOR = re.compile(
    r"(?:请|建议|推荐|应|应当|应该|需要|必须|务必|现在|立即|尽快|马上|尽早|立刻|赶紧|快)"
    r"(?:你|您)?(?:现在|立即|尽快|马上|尽早|立刻|赶紧|就|直接)?"
    r"(?:前往|去|到|来|就医|拨打|呼叫|挂急诊|看急诊)"
    r"|(?:让|请|由|叫|务必)?[^。！？；;!\n]{0,14}?(?:陪同|陪)[^。！？；;!\n]{0,8}(?:前往|去|到|来|就医|看急诊|挂急诊)"
    r"|非[^。！？；;!\n]{0,6}不可"
    r"|(?:^|[，,：:；;])(?:前往|去|到|来|就医|看急诊|挂急诊)"
)


def _next_sentence_head(text: str, after: int) -> str:
    """Return the leading content of the sentence following position `after` (capped)."""
    head = text[after:]
    head = head.lstrip(" 　，,。！？；;!\n")
    for separator in "。！？；;!\n":
        ...
        if index != -1:
            head = head[:index]
            break
    return head[:_NEXT_SENTENCE_HEAD_CHARS]
...
def _match_escalation_phrases(output: str, phrases: list[str]) -> tuple[list[str], list[str]]:
    ...
    for phrase in phrases:
        ...
        for match in matches:
            ...
            # 歧义检查必须覆盖整句（含短语本身），否则 "尽快就医这个问题很重要" 这类
            # 紧跟短语的元讨论会逃过 "的问题/的时机" 检查。
            context = prefix + phrase + suffix
            next_head = _next_sentence_head(output, match.end() + len(suffix))
            if _ESCALATION_QUESTION_OR_MODAL.search(context):
                continue
            if _ESCALATION_ALTERNATIVE.search(context):
                continue
            if _ESCALATION_CONDITION.search(context):
                continue
            if _ESCALATION_DEFER_IMMEDIATE.search(context):
                continue
            if _ESCALATION_DEFER_TIME.search(context):
                continue
            if _ESCALATION_NEXT_SENTENCE_VETO.search(next_head):
                continue
            # 锚点检查整句（含后缀），因为 "非去急诊不可" 的 "不可" 在短语之后；
            # 元讨论/软化已在上述 veto 中先被拒绝，不会借后缀内容误锚。
            if not _ESCALATION_ANCHOR.search(prefix + phrase + suffix):
                continue
            positive = True
            break
        if positive:
