...
def is_negated(text: str, match: re.Match[str]) -> bool:
    ...
    # G4 细粒度修复：后缀逗号不截断（2026-08-20 逗号截断重评）。
    # 旧逻辑在逗号处截断后缀，导致"尽快就医，后续观察即可"中逗号后的
    # 软化/推迟表述（后续观察/观察即可/在家观察等）丢失，is_negated 误判
    # 为未否定。M3.1 的升级匹配器已用跨句 veto 兜住，M2 沿用旧行为至此。
    # 细粒度策略：前缀仍在逗号处隔离（"先观察，尽快就医"不跨逗号否定），
    # 后缀仅在句末标点处截断，保留逗号后 16 字符窗口内的软化证据。
    for separator in ("。", "！", "？", "；", ";", "\n"):
